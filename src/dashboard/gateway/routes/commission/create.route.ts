...
export const route = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
	try {
		const params = { ...req.params, ...req.query, ...req.body } as ICommission;
		...
		await CommissionValidator.isNotExists({ cryptoCurrency });

		const commissionService = new CommissionService();
		const doc = await commissionService.create(params);
