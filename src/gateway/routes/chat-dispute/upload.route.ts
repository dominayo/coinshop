...
import ChatValidator from '../../../services/chat-dispute/validator';
...
export const route = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
	try {
		...
		await ChatValidator.isChatDisputeParticipant({ id: chatId, userId });
		...
		form.parse(req, async (err, fields: Fields, files: Files) => {
			try {
				...
				// const chatMessageDoc = await chatDisputeMessageService.create(
				// 	{ chatId, userId, message: name, messageType: MessageType.File, contentType, role: UserRoles.Client });

				// const dto = _.pick(chatMessageDoc,
				// 	['id', 'dealId', 'chatId', 'message', 'type', 'contentType', 'messageType', 'createdAt', 'updatedAt']);
